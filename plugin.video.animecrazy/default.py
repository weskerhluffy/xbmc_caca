...
def loadVideos(url,name,isRequestForURL,isRequestForPlaylist):
        ...
        try:
                ...
                H=re.compile('&H=(.+?)&').findall(url)[0]
                downloadUrl = 'http://www.zshare.net/download/' + H + '/'
                print downloadUrl
                ...
                response.close()
                link = ''.join(link.splitlines()).replace('\t','').replace('\'','"')
                referrer = re.compile('<input name="referer2" type="hidden" id="referer2" value="(.+?)">').findall(link)[0]
                id = re.compile('<input name="download" type="hidden" id="download" value="(.+?)">').findall(link)[0]
        
                values = {'referer2': referrer,'download': id}
                headers = { 'User-Agent' : 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3' }
                data = urllib.urlencode(values)
                ...
                response.close()
                link = ''.join(link.splitlines()).replace('\t','').replace('\'','"')
                link_enc = re.compile('var link_enc=new Array\((.+?)\);').findall(link)[0]
                link_chars = re.compile('"(.+?)"').findall(link_enc)
                link = ''
                for chars in link_chars:
                        link = link + chars
                #print link
                display_wait(50,'Z-Share - Retieving Video Link','I know you can\'t wait but thats the way it works.')
